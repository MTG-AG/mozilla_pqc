...
seckey_GetKeyType(SECOidTag tag)
{
    ...
    if (mtg_get_key_type_for_sec_tag(tag, &keyType) == SECSuccess)
        return keyType;
...
seckey_ExtractPublicKey(const CERTSubjectPublicKeyInfo *spki)
{
    ...
    if (rv == SECSuccess && mtg_get_public_key_from_spki(pubk, tag, &newOs, &spki->algorithm.parameters) == SECSuccess)
        return pubk;
...
SECKEY_PublicKeyStrengthInBits(const SECKEYPublicKey *pubk)
{
    ...
    if (mtg_get_public_key_strength_in_bits(pubk->keyType, &bitSize) == SECSuccess)
        return bitSize;
...
SECKEY_SignatureLen(const SECKEYPublicKey *pubk)
{
    ...
    if (mtg_get_signature_len(pubk, &size) == SECSuccess)
        return size;
...
SECKEY_CopyPublicKey(const SECKEYPublicKey *pubk)
{
    ...
    if (mtg_copy_public_key(pubk, copyk) == SECSuccess)
        return copyk;
...
seckey_CreateSubjectPublicKeyInfo_helper(SECKEYPublicKey *pubk)
{
    ...
    if (spki != NULL) {
        ...
        if (mtg_get_CERTSubjectPublicKeyInfo_from_SECKEYPublicKey(pubk, &params, spki) == SECSuccess)
            return spki;
...
SECKEY_ImportDERPublicKey(const SECItem *derKey, CK_KEY_TYPE type)
{
    ...
    if (mtg_import_der_public_key(type, &newDerKey, pubk) == SECSuccess)
        return pubk;
