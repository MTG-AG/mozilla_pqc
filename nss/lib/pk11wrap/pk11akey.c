...
PK11_ExtractPublicKey(PK11SlotInfo *slot, KeyType keyType, CK_OBJECT_HANDLE id)
{
    ...
    PK11_SETATTRS(attrs, CKA_KEY_TYPE, &pk11KeyType,
                  sizeof(pk11KeyType));
    attrs++;
    if (mtg_retrieve_public_key_from_pkcs11(pubKey, tmp_arena, attrs, template, slot, id, &pk11KeyType, &keyClass, &crv) != SECSuccess)
    {
        switch (pubKey->keyType)
        {
            case rsaKey:
                                ...
                                CK_FLAGS opFlags, CK_FLAGS opFlagsMask, void *wincx)
{
    ...
    if (mtg_prepare_attrs_for_keygen(type, param, &attrs, &pubTemplate, &keyType, &test_mech) != SECSuccess)
    {
        switch (type)
        {
                                               ...
                                               void *wincx)
{
    ...
    if (mtg_set_priv_key_usage(keyType, keyUsage, &usage, &usageCount) != SECSuccess)
    {
        switch (keyType)
        {
            default:
