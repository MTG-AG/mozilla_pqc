...
sec_pkcs12_add_cert(sec_PKCS12SafeBag *cert, PRBool keyExists, void *wincx)
{
    ...
    if (keyExists) {
        ...
        if ((rv = mtg_import_cert_for_key_to_slot(newCert, nickData, PR_TRUE, wincx)) != SECSuccess)
        {
                   ...
                   SECItem *nickName, PRBool forceUnicode, void *wincx)
{
    ...
    switch (SECOID_FindOIDTag(&key->safeBagType)) {
        ...
        case SEC_OID_PKCS12_V1_PKCS8_SHROUDED_KEY_BAG_ID: {
            ...
            if ((rv = mtg_import_encrypted_private_key_info(key->safeBagContent.pkcs8ShroudedKeyBag,
                    &pwitem, nickName, publicValue, PR_TRUE, PR_TRUE, keyType, keyUsage, wincx)) != SECSuccess)
            {
    ...
    if (rv != SECSuccess) {
        ...
        if (pubKey) {
            if ((rv = mtg_import_public_key(pubKey, PR_TRUE, wincx)) != SECSuccess)
            {
