...
{
    switch (cipherSuite) {
        MTG_GET_CIPHER_SUITES_TLS_VERSION_REQ(vrange)
...
{
    switch (keaType) {
        MTG_GET_KEA_ENABLED(ss, keaType)
        case ssl_kea_rsa:
            return PR_TRUE;
                            ...
                            PK11SymKey **msp)
{
    ...
    MTG_SET_KEA_AS_DH(ss->ssl3.hs.kea_def->exchKeyType);
...
ssl3_SendClientKeyExchange(sslSocket *ss)
{
    ...
    switch (ss->ssl3.hs.kea_def->exchKeyType) {
        ...
        case ssl_kea_ecdh:
            ...
            break;
        MTG_SEND_CLIENT_KEY_EXCHANGE(ss, serverKey)
        default:
            PORT_Assert(0);
            PORT_SetError(SEC_ERROR_LIBRARY_FAILURE);
            break;
...
ssl3_HandleServerKeyExchange(sslSocket *ss, PRUint8 *b, PRUint32 length)
{
    ...
    switch (ss->ssl3.hs.kea_def->exchKeyType) {
        ...
        case ssl_kea_ecdh:
            ...
            break;
        MTG_HANDLE_SERVER_KEY_EXCHANGE(ss, b, length)
        default:
...
#define MAX_HANDSHAKE_MSG_LEN 0x1ffff /* 128k - 1 */ + MTG_MAX_HANDSHAKE_MSG_LEN  /*0xffE0000*/
